                ...
                {(order.chiTietDonHangList || order.chiTietDonHangs || order.items || order.orderDetails || []).map((item, index) => (
                  <div key={index} className="flex items-center gap-4 p-4 bg-gray-50 rounded-lg">
                    ...
                    <div className="text-right">
                      <p className="text-sm text-gray-600">
                        Số lượng: {item.soLuong || item.quantity || 0}
                      </p>
                      <p className="font-semibold text-gray-900">
                        {formatCurrency(item.donGia || item.donGiaThucTe || item.price || 0)}
                      </p>
                      <p className="text-sm font-bold text-blue-600">
                        = {formatCurrency((item.donGia || item.donGiaThucTe || item.price || 0) * (item.soLuong || item.quantity || 0))}
                      </p>
                    </div>
                  </div>
                ))}
