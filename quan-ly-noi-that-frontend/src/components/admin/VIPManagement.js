...
const VIPManagement = () => {
  ...
  useEffect(() => {
    const fetchVipData = async () => {
      ...
      try {
        // Fetch VIP levels
        const levelsData = await api.get('/api/vip/levels');
        if (Array.isArray(levelsData)) {
          setVipBenefits(levelsData.map(mapVipLevelFromApi));
        }

        // Fetch VIP customers
        const customersData = await api.get('/api/vip/customers');
        if (Array.isArray(customersData)) {
          setVipCustomers(customersData.map(mapVipCustomerFromApi));
        }
      } catch (err) {
