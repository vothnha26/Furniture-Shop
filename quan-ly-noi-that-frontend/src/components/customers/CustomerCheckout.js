...
const CustomerCheckout = ({ onBack, onOrderComplete }) => {
  ...
  const [lastOrderPreview, setLastOrderPreview] = useState(null);
  ...
  chiTietDonHangList: cartItems.map(i => ({ maBienThe: i.variantId ?? i.id, soLuong: i.quantity })),
      ...
      const returnedOrderCode = resp?.maDonHangStr ?? resp?.maDonHang ?? ('DH' + Date.now().toString().slice(-6));
      setOrderCode(returnedOrderCode);
      // keep lastOrderResponse as resp so we can get id when navigating
      setLastOrderResponse(resp || null);
          ...
          <div className="bg-white rounded-lg shadow-lg p-8 text-center">
            ...
            <div className="bg-gray-50 rounded-lg p-6 mb-6">
              ...
              <div className="flex items-center justify-between mb-4">
                ...
                <span className="font-bold text-lg">{formatCurrency((lastOrderResponse && (lastOrderResponse.thanhTien ?? lastOrderResponse.ThanhTien)) ?? (lastOrderPreview?.tongCong ?? calculateTotal()))}</span>
              </div>
            ...
            <div className="bg-white rounded-lg p-4 border border-gray-100 mb-6">
              <div className="flex items-center justify-between">
                ...
                <div className="text-right">
                  <div className="font-medium text-green-700">{(lastOrderPreview?.DiemThuongNhanDuoc ?? lastOrderPreview?.diemThuongNhanDuoc ?? getPointsEarned())} điểm</div>
                  <div className="text-xs text-gray-500">~ {formatCurrency(((lastOrderPreview?.DiemThuongNhanDuoc ?? lastOrderPreview?.diemThuongNhanDuoc ?? getPointsEarned()) || 0) * 1000)}</div>
                </div>
              </div>
            </div>
            ...
            <div className="bg-white rounded-lg shadow-sm p-6 sticky top-6">
              ...
              <div className="space-y-3 mb-6">
                ...
                <div className="flex justify-between">
                  ...
                  <span>{serverPreview?.phiGiaoHang || 'Miễn phí'}</span>
                </div>
