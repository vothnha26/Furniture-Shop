...
const AttributeValueManagement = () => {
  ...
  useEffect(() => {
    const fetchAttributeValues = async () => {
      setIsLoading(true);
      try {
        const data = await api.get('/api/attributes/1/values'); // Fixed: use AttributeController endpoint
        if (Array.isArray(data)) {
          setAttributeValues(data.map(mapAttributeValueFromApi));
        }
      } catch (err) {
        console.error('Fetch attribute values error', err);
        setError(err);
      } finally {
        setIsLoading(false);
      }
  ...
  const [attributeValues, setAttributeValues] = useState([
    { maGiaTri: 1, thuocTinh: { maThuocTinh: 1, tenThuocTinh: 'Màu sắc' }, giaTri: 'Đỏ' },
    { maGiaTri: 2, thuocTinh: { maThuocTinh: 1, tenThuocTinh: 'Màu sắc' }, giaTri: 'Xanh' },
    { maGiaTri: 3, thuocTinh: { maThuocTinh: 1, tenThuocTinh: 'Màu sắc' }, giaTri: 'Vàng' },
    { maGiaTri: 4, thuocTinh: { maThuocTinh: 2, tenThuocTinh: 'Chất liệu' }, giaTri: 'Gỗ sồi' },
    { maGiaTri: 5, thuocTinh: { maThuocTinh: 2, tenThuocTinh: 'Chất liệu' }, giaTri: 'Gỗ thông' },
    { maGiaTri: 6, thuocTinh: { maThuocTinh: 2, tenThuocTinh: 'Chất liệu' }, giaTri: 'Kim loại' },
    { maGiaTri: 7, thuocTinh: { maThuocTinh: 3, tenThuocTinh: 'Kích thước' }, giaTri: 'S' },
    { maGiaTri: 8, thuocTinh: { maThuocTinh: 3, tenThuocTinh: 'Kích thước' }, giaTri: 'M' },
    { maGiaTri: 9, thuocTinh: { maThuocTinh: 3, tenThuocTinh: 'Kích thước' }, giaTri: 'L' }
  ]);
  ...
  const handleSubmit = (e) => {
    ...
    const exists = attributeValues.some(value => 
      value.thuocTinh.maThuocTinh.toString() === formData.maThuocTinh &&
      value.giaTri.toLowerCase() === formData.giaTri.toLowerCase() &&
      value.maGiaTri !== editingValue?.maGiaTri
    );

    if (exists) {
      showToast('Giá trị này đã tồn tại cho thuộc tính được chọn', 'error');
      return;
    }
    ...
    if (editingValue) {
      ...
      setAttributeValues(attributeValues.map(value =>
        value.maGiaTri === editingValue.maGiaTri
          ? { 
              ...value, 
              thuocTinh: selectedAttr,
      ...
      // Add new value
      const newValue = {
        maGiaTri: Math.max(...attributeValues.map(v => v.maGiaTri), 0) + 1,
        thuocTinh: selectedAttr,
        giaTri: formData.giaTri
      };
  ...
  const confirmDelete = () => {
    setAttributeValues(attributeValues.filter(value => value.maGiaTri !== deleteId));
