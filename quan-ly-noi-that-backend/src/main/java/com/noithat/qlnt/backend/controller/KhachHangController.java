...
public class KhachHangController {
    ...
    // [Quyền: Khách hàng (Auth)] - Cập nhật profile của chính mình
    @PutMapping("/me")
    public ResponseEntity<KhachHang> updateMyProfile(java.security.Principal principal, @RequestBody java.util.Map<String, Object> payload) {
        if (principal == null) {
            return ResponseEntity.status(401).build();
        }
        String username = principal.getName();
        try {
            var maybe = khachHangRepository.findByTaiKhoan_TenDangNhap(username);
            if (maybe == null || maybe.isEmpty()) {
                return ResponseEntity.status(404).build();
            }
            Integer maKh = maybe.get().getMaKhachHang();

            // Build a KhachHang object with only updatable fields to pass to service.update
            KhachHang updateReq = new KhachHang();
            if (payload.containsKey("hoTen")) updateReq.setHoTen(String.valueOf(payload.get("hoTen")));
            if (payload.containsKey("email")) updateReq.setEmail(String.valueOf(payload.get("email")));
            if (payload.containsKey("soDienThoai")) updateReq.setSoDienThoai(String.valueOf(payload.get("soDienThoai")));
            if (payload.containsKey("diaChi")) updateReq.setDiaChi(String.valueOf(payload.get("diaChi")));
            // If frontend sent ngaySinh (ISO yyyy-MM-dd), parse and set it
            if (payload.containsKey("ngaySinh")) {
                try {
                    String s = String.valueOf(payload.get("ngaySinh"));
                    if (s != null && !s.isBlank()) {
                        java.time.LocalDate d = java.time.LocalDate.parse(s);
                        updateReq.setNgaySinh(d);
                    }
                } catch (Exception ex) {
                    System.err.println("Invalid ngaySinh format: " + payload.get("ngaySinh"));
                }
            }

            KhachHang updated = khachHangService.update(maKh, updateReq);
